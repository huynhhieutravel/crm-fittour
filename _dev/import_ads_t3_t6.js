...
const processFiles = async () => {
  const filePath = `../client/public/thu-vien-input/qc-t3-t6-2026.xlsx`;
  ...
  for (const row of data) {
    ...
    let spendRaw = (row['Số tiền đã chi tiêu (VND)'] || '0').toString().replace(/[^0-9.-]+/g,"");
    const spend = parseFloat(spendRaw) || 0;
    const msgs = parseInt(row['Lượt bắt đầu cuộc trò chuyện qua tin nhắn'] || '0') || 0;
    const leads = parseInt(row['Khách hàng tiềm năng'] || '0') || 0;
    ...
    if (bu !== 'UNKNOWN' && (spend > 0 || campaign)) {
      ...
      await pool.query(
        ...
        [bu, YEAR, MONTH, WEEK, row['Tên chiến dịch'] || '', row['Tên nhóm quảng cáo'] || '', row['Tên quảng cáo'] || '', spend, msgs, leads, cplMsg, cplLead]
      );
      count++;
