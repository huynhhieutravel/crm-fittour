...
import { X, Save, Plus, Trash2, Shield, ShoppingBag, Users, FileText, Send, Clock, PlusCircle , ExternalLink, Link2 } from 'lucide-react';
import Select from 'react-select';
import { useMarkets } from '../../hooks/useMarkets';
import { isViewOnly as checkViewOnly } from '../../utils/permissions';

export default function InsuranceDetailDrawer({ insurance, onClose, refreshList, currentUser, checkPerm, addToast }) {
    ...
    const [formData, setFormData] = useState({
        ...
        website: '', insurance_class: '', market: '', drive_link: '',
    ...
    useEffect(() => {
        if (insurance) {
            setFormData({
                ...
                market: insurance.market || '', drive_link: insurance.drive_link || '',
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                            ...
                            <div style={{ background: 'white', padding: '2rem', borderRadius: '12px', border: '1px solid #e2e8f0', boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
                                ...
                                <div className="mobile-stack-grid mobile-stack-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.25rem 2rem' }}>
                                    <div>
                                        <label style={labelStyle}>Mã NCC Bảo Hiểm (Tự Động)</label>
                                        <input type="text" style={{ ...drawerInputStyle, background: '#f1f5f9', color: '#64748b', fontWeight: 600 }} value={formData.code} onChange={e => setFormData({...formData, code: e.target.value})} disabled={isViewOnly} placeholder="Để trống tự động tạo INS-001" />
                                    </div>
                                    <div>
