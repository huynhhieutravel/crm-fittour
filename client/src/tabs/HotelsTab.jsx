...
export default function HotelsTab({ currentUser, checkPerm, addToast, handleDeleteHotel }) {
    ...
    const handleBulkDelete = async () => {
        ...
        for (const id of selectedIds) {
            try {
                await axios.delete(`/api/hotels/${id}?force=true`, { headers: { Authorization: `Bearer ${token}` } });
                successCount++;
            } catch (err) {
                console.error(err);
                failCount++;
                ...
                <table className="data-table" style={{ width: '100%', borderCollapse: 'collapse' }}>
                    <thead style={{ background: '#f8fafc', borderBottom: '2px solid #e2e8f0' }}>
                        <tr style={{ color: '#475569', fontSize: '0.8rem' }}>
                            <th style={{ padding: '16px 20px', textAlign: 'center', width: '50px' }}>
                                <input type="checkbox" checked={hotels.length > 0 && selectedIds.length === hotels.length} onChange={(e) => setSelectedIds(e.target.checked ? hotels.map(item => item.id) : [])} style={{ width: '16px', height: '16px', cursor: 'pointer', accentColor: '#2563eb' }} />
                            </th>
                            <th style={{ padding: '16px 20px', textAlign: 'left' }}>KHÁCH SẠN</th>
                    ...
                    <tbody>
                        {loading ? (
                            ...
                            hotels.map(h => (
                                <tr key={h.id} className="table-row-hover" style={{ transition: 'background 0.2s', cursor: 'pointer' }} onClick={() => handleOpenHotel(h.id)} onMouseOver={e=>e.currentTarget.style.background='#f8fafc'} onMouseOut={e=>e.currentTarget.style.background='white'}>
                                    ...
                                    <td style={{ padding: '16px 20px', borderBottom: '1px solid #f1f5f9' }}>
                                        <span style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', background: '#fffbeb', color: '#b45309', border: '1px solid #fef3c7', padding: '4px 10px', borderRadius: '6px', fontSize: '0.8rem', fontWeight: 600 }}>
                                    ...
                                    <td style={{ padding: '16px 20px', borderBottom: '1px solid #f1f5f9', textAlign: 'center' }} onClick={e => e.stopPropagation()}>
                                        <div style={{ display: 'flex', gap: '6px', justifyContent: 'center', alignItems: 'center' }}>
                                            <button className="btn-action btn-edit-pro" title="Xem / Sửa" onClick={() => handleOpenHotel(h.id)}>
                                                <Edit2 size={15} />
                                            </button>
                                            {(checkPerm ? checkPerm('hotels', 'delete') : canDelete(currentUser?.role, 'suppliers')) && (
                                                <button className="btn-action btn-delete-pro" title="Xoá" onClick={() => handleDeleteHotel(h.id)}>
                                                    <Trash2 size={15} />
                                                </button>
                                            )}
                                        </div>
