...
const INSURANCE_CLASS_OPTIONS = [
    ...
    { value: 'partial_package', label: 'Bảo Hiểm Từng Phần' },
    { value: 'join_in', label: 'Bảo Hiểm Ghép' },
...
export default function InsurancesTab({ currentUser, checkPerm, addToast, handleDeleteInsurance }) {
    ...
    useEffect(() => {
        const handleReload = () => fetchInsurances();
        window.addEventListener('reloadInsurances', handleReload);
        return () => window.removeEventListener('reloadInsurances', handleReload);
    }, [search, marketFilter, starFilter]);
    ...
    const handleOpenInsurance = async (insuranceId) => {
        try {
            setActionLoading(true);
            const token = localStorage.getItem('token');
            const res = await axios.get(`/api/insurances/${insuranceId}`, {
                headers: { Authorization: `Bearer ${token}` }
            });
            setSelectedInsurance(res.data);
            setIsDrawerOpen(true);
        } catch (err) {
            console.error('Lỗi khi lấy chi tiết bảo hiểm', err);
            if (addToast) addToast('Lỗi lấy thông tin bảo hiểm: ' + (err.response?.data?.message || err.message), 'error');
    ...
    const handleAddInsurance = () => {
        setSelectedInsurance(null); // Tự động tạo mới
        setIsDrawerOpen(true);
    };
    ...
    const handleBulkDelete = async () => {
        ...
        for (const id of selectedIds) {
            try {
                await axios.delete(`/api/insurances/${id}?force=true`, { headers: { Authorization: `Bearer ${token}` } });
                successCount++;
            } catch (err) {
                console.error(err);
                failCount++;
    ...
    return (
        <div style={{ padding: '0 22px' }}>
                ...
                <div className="ncc-filter-grid">
                    <div className="filter-group" style={{ margin: 0 }}>
                        ...
                        <div style={{ position: 'relative' }}>
                            ...
                            <input className="filter-input" style={{ width: '100%', paddingLeft: '40px', height: '44px', borderRadius: '8px', border: '1px solid #cbd5e1', outline: 'none' }} type="text" placeholder="Mã, Tên bảo hiểm..." value={search} onChange={(e) => setSearch(e.target.value)} />
                        </div>
                    </div>
                    <div className="filter-group" style={{ margin: 0 }}>
                    ...
                    <div className="filter-group" style={{ margin: 0 }}>
                        ...
                        <Select options={INSURANCE_CLASS_OPTIONS} value={INSURANCE_CLASS_OPTIONS.find(o => o.value === starFilter) || null} onChange={option => setStarFilter(option ? option.value : '')} styles={reactSelectStyles} isClearable placeholder="Tất cả" />
                    </div>
                    <div style={{ display: 'flex', alignItems: 'flex-end' }}>
                        ...
                        {(checkPerm ? checkPerm('insurances', 'create') : canCreate(currentUser?.role, 'suppliers')) && (
                            <button className="btn btn-primary" onClick={handleAddInsurance} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', height: '44px', padding: '0 1.5rem', borderRadius: '8px', fontSize: '0.9rem', fontWeight: 700, background: '#2563eb', color: 'white', border: 'none', boxShadow: '0 4px 6px rgba(37, 99, 235, 0.2)', cursor: 'pointer', whiteSpace: 'nowrap' }}>
                    ...
                    <thead style={{ background: '#f8fafc', borderBottom: '2px solid #e2e8f0' }}>
                        <tr style={{ color: '#475569', fontSize: '0.8rem' }}>
                            <th style={{ padding: '16px 20px', textAlign: 'center', width: '50px' }}>
                                <input type="checkbox" checked={insurances.length > 0 && selectedIds.length === insurances.length} onChange={(e) => setSelectedIds(e.target.checked ? insurances.map(item => item.id) : [])} style={{ width: '16px', height: '16px', cursor: 'pointer', accentColor: '#2563eb' }} />
                            </th>
                            <th style={{ padding: '16px 20px', textAlign: 'left' }}>BẢO HIỂM</th>
                        ...
                        {loading ? (
                            ...
                            <tr>
                                <td colSpan="8" style={{ textAlign: 'center', padding: '3rem', color: '#64748b' }}>
                                    Không có bảo hiểm nào khớp với tìm kiếm.
                                </td>
                            </tr>
                        ) : (
                            insurances.map(h => (
                                <tr key={h.id} className="table-row-hover" style={{ transition: 'background 0.2s', cursor: 'pointer' }} onClick={() => handleOpenInsurance(h.id)} onMouseOver={e=>e.currentTarget.style.background='#f8fafc'} onMouseOut={e=>e.currentTarget.style.background='white'}>
                                    ...
                                    <td style={{ padding: '16px 20px', borderBottom: '1px solid #f1f5f9' }}>
                                        <span style={{ background: '#fff7ed', color: '#c2410c', border: '1px solid #ffedd5', padding: '4px 10px', borderRadius: '6px', fontSize: '0.8rem', fontWeight: 600 }}>
                                            {h.insurance_class === 'full_package' ? 'Bảo Hiểm Trọn Gói' : 
                                             h.insurance_class === 'partial_package' ? 'Bảo Hiểm Từng Phần' : 
                                             h.insurance_class === 'join_in' ? 'Bảo Hiểm Ghép' : 'Khác'}
                                        </span>
                                    </td>
                                    ...
                                    <td style={{ padding: '16px 20px', borderBottom: '1px solid #f1f5f9', textAlign: 'center' }} onClick={e => e.stopPropagation()}>
                                        <div style={{ display: 'flex', gap: '6px', justifyContent: 'center', alignItems: 'center' }}>
                                            <button className="btn-action btn-edit-pro" title="Xem / Sửa" onClick={() => handleOpenInsurance(h.id)}>
                                                <Edit2 size={15} />
                                            </button>
                                            {(checkPerm ? checkPerm('insurances', 'delete') : canDelete(currentUser?.role, 'suppliers')) && (
                                                <button className="btn-action btn-delete-pro" title="Xoá" onClick={() => handleDeleteInsurance(h.id)}>
                                                    <Trash2 size={15} />
                                                </button>
                                            )}
                                        </div>
            ...
            {/* Khung Drawer chi tiết Bảo Hiểm */}
            {isDrawerOpen && (
                <InsuranceDetailDrawer 
                    insurance={selectedInsurance} 
                    onClose={() => setIsDrawerOpen(false)} 
                    refreshList={fetchInsurances}
