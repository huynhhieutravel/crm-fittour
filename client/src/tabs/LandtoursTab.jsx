                ...
                <table className="data-table" style={{ width: '100%', borderCollapse: 'collapse' }}>
                    <thead style={{ background: '#f8fafc', borderBottom: '2px solid #e2e8f0' }}>
                        <tr style={{ color: '#475569', fontSize: '0.8rem' }}>
                            <th style={{ padding: '16px 20px', textAlign: 'center', width: '50px' }}>
                                <input type="checkbox" checked={landtours.length > 0 && selectedIds.length === landtours.length} onChange={(e) => setSelectedIds(e.target.checked ? landtours.map(item => item.id) : [])} style={{ width: '16px', height: '16px', cursor: 'pointer', accentColor: '#2563eb' }} />
                            </th>
                            <th style={{ padding: '16px 20px', textAlign: 'left' }}>LAND TOUR</th>
                    ...
                    <tbody>
                        {loading ? (
                            ...
                            landtours.map(h => (
                                <tr key={h.id} className="table-row-hover" style={{ transition: 'background 0.2s', cursor: 'pointer' }} onClick={() => handleOpenLandtour(h.id, 'view')} onMouseOver={e=>e.currentTarget.style.background='#f8fafc'} onMouseOut={e=>e.currentTarget.style.background='white'}>
                                    ...
                                    <td style={{ padding: '16px 20px', borderBottom: '1px solid #f1f5f9', textAlign: 'center' }} onClick={e => e.stopPropagation()}>
                                        <div style={{ display: 'flex', gap: '6px', justifyContent: 'center', alignItems: 'center' }}>
                                            <button className="btn-action btn-view-pro" title="Xem" onClick={() => handleOpenLandtour(h.id, 'view')}>
                                                <Eye size={15} />
                                            </button>
                                            <button className="btn-action btn-edit-pro" title="Sửa" onClick={() => handleOpenLandtour(h.id, 'edit')}>
                                                <Edit2 size={15} />
                                            </button>
                                            {(checkPerm ? checkPerm('landtours', 'delete') : canDelete(currentUser?.role, 'suppliers')) && (
                                                <button className="btn-action btn-delete-pro" title="Xoá" onClick={() => handleDeleteLandtour(h.id)}>
                                                    <Trash2 size={15} />
                                                </button>
                                            )}
                                        </div>
