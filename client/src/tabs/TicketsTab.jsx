...
export default function TicketsTab({ currentUser, checkPerm, addToast, handleDeleteTicket }) {
    ...
    const handleBulkDelete = async () => {
        ...
        for (const id of selectedIds) {
            try {
                await axios.delete(`/api/tickets/${id}?force=true`, { headers: { Authorization: `Bearer ${token}` } });
                successCount++;
            } catch (err) {
                console.error(err);
                failCount++;
                ...
                <div className="ncc-filter-grid">
                    <div className="filter-group" style={{ margin: 0 }}>
                        ...
                        <div style={{ position: 'relative' }}>
                            ...
                            <input className="filter-input" style={{ width: '100%', paddingLeft: '40px', height: '44px', borderRadius: '8px', border: '1px solid #cbd5e1', outline: 'none' }} type="text" placeholder="Mã, Tên vé tham quan..." value={search} onChange={(e) => setSearch(e.target.value)} />
                        </div>
                    </div>
                    <div className="filter-group" style={{ margin: 0 }}>
                    ...
                    <div className="filter-group" style={{ margin: 0 }}>
                        ...
                        <Select options={TRANSPORT_CLASS_OPTIONS} value={TRANSPORT_CLASS_OPTIONS.find(o => o.value === starFilter) || null} onChange={option => setStarFilter(option ? option.value : '')} styles={reactSelectStyles} isClearable placeholder="Tất cả" />
                    </div>
                    <div style={{ display: 'flex', alignItems: 'flex-end' }}>
                        ...
                        {(checkPerm ? checkPerm('tickets', 'create') : canCreate(currentUser?.role, 'suppliers')) && (
                            <button className="btn btn-primary" onClick={handleAddTicket} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', height: '44px', padding: '0 1.5rem', borderRadius: '8px', fontSize: '0.9rem', fontWeight: 700, background: '#2563eb', color: 'white', border: 'none', boxShadow: '0 4px 6px rgba(37, 99, 235, 0.2)', cursor: 'pointer', whiteSpace: 'nowrap' }}>
                ...
                <table className="data-table" style={{ width: '100%', borderCollapse: 'collapse' }}>
                    <thead style={{ background: '#f8fafc', borderBottom: '2px solid #e2e8f0' }}>
                        <tr style={{ color: '#475569', fontSize: '0.8rem' }}>
                            <th style={{ padding: '16px 20px', textAlign: 'center', width: '50px' }}>
                                <input type="checkbox" checked={tickets.length > 0 && selectedIds.length === tickets.length} onChange={(e) => setSelectedIds(e.target.checked ? tickets.map(item => item.id) : [])} style={{ width: '16px', height: '16px', cursor: 'pointer', accentColor: '#2563eb' }} />
                            </th>
                            <th style={{ padding: '16px 20px', textAlign: 'left' }}>VÉ THAM QUAN</th>
                    ...
                    <tbody>
                        {loading ? (
                            ...
                            tickets.map(h => (
                                <tr key={h.id} className="table-row-hover" style={{ transition: 'background 0.2s', cursor: 'pointer' }} onClick={() => handleOpenTicket(h.id)} onMouseOver={e=>e.currentTarget.style.background='#f8fafc'} onMouseOut={e=>e.currentTarget.style.background='white'}>
                                    ...
                                    <td style={{ padding: '16px 20px', borderBottom: '1px solid #f1f5f9' }}>
                                        <span style={{ background: '#fff7ed', color: '#c2410c', border: '1px solid #ffedd5', padding: '4px 10px', borderRadius: '6px', fontSize: '0.8rem', fontWeight: 600 }}>
                                            {h.ticket_class === 'adult' ? 'Người lớn' : 
                                             h.ticket_class === 'child' ? 'Trẻ em' : 
                                    ...
                                    <td style={{ padding: '16px 20px', borderBottom: '1px solid #f1f5f9', textAlign: 'center' }} onClick={e => e.stopPropagation()}>
                                        <div style={{ display: 'flex', gap: '6px', justifyContent: 'center', alignItems: 'center' }}>
                                            <button className="btn-action btn-edit-pro" title="Xem / Sửa" onClick={() => handleOpenTicket(h.id)}>
                                                <Edit2 size={15} />
                                            </button>
                                            {(checkPerm ? checkPerm('tickets', 'delete') : canDelete(currentUser?.role, 'suppliers')) && (
                                                <button className="btn-action btn-delete-pro" title="Xoá" onClick={() => handleDeleteTicket(h.id)}>
                                                    <Trash2 size={15} />
                                                </button>
                                            )}
                                        </div>
