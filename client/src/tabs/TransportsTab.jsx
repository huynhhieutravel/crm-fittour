...
export default function TransportsTab({ currentUser, checkPerm, addToast, handleDeleteTransport }) {
    ...
    useEffect(() => {
        ...
        window.addEventListener('reloadTransports', handleReload);
        return () => window.removeEventListener('reloadTransports', handleReload);
    }, [search, marketFilter, starFilter]);
    ...
    const handleOpenTransport = async (transportId) => {
        try {
            ...
            const token = localStorage.getItem('token');
            const res = await axios.get(`/api/transports/${transportId}`, {
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    const handleBulkDelete = async () => {
        ...
        for (const id of selectedIds) {
            try {
                await axios.delete(`/api/transports/${id}?force=true`, { headers: { Authorization: `Bearer ${token}` } });
                successCount++;
            } catch (err) {
                console.error(err);
                failCount++;
                ...
                <div className="ncc-filter-grid">
                    <div className="filter-group" style={{ margin: 0 }}>
                        ...
                        <div style={{ position: 'relative' }}>
                            ...
                            <input className="filter-input" style={{ width: '100%', paddingLeft: '40px', height: '44px', borderRadius: '8px', border: '1px solid #cbd5e1', outline: 'none' }} type="text" placeholder="Mã, Tên nhà xe..." value={search} onChange={(e) => setSearch(e.target.value)} />
                        </div>
                    </div>
                    <div className="filter-group" style={{ margin: 0 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'flex-end' }}>
                        ...
                        {(checkPerm ? checkPerm('transports', 'create') : canCreate(currentUser?.role, 'suppliers')) && (
                            <button className="btn btn-primary" onClick={handleAddTransport} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', height: '44px', padding: '0 1.5rem', borderRadius: '8px', fontSize: '0.9rem', fontWeight: 700, background: '#2563eb', color: 'white', border: 'none', boxShadow: '0 4px 6px rgba(37, 99, 235, 0.2)', cursor: 'pointer', whiteSpace: 'nowrap' }}>
