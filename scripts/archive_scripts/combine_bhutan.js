...
// Combine the data filters
code = code.replace(
    /const filteredMarketingLinks[\s\S]*?normalizedSearch\)\)\n    \);/g,
    `const filteredMarketingLinks = referenceLinks.filter(link => 
        link.type === 'marketing' && (link.title.toLowerCase().includes(normalizedSearch) || 
        link.desc.toLowerCase().includes(normalizedSearch))
    );` // wait, if I just replace it with filteredAllLinks, let me do that properly
);

// Actually, let's just create a filteredAllLinks variable, and use that.
code = code.replace(
    /const filteredTourLinks = referenceLinks\.filter\(link =>[\s\S]*?normalizedSearch\)\)\n    \);/,
...
// We need to replace the layout block. 
// Let's capture the marketing block
const marketingRegex = /\{\/\* ====== Reference Links Section ====== \*\/\}[\s\S]*?\{\/\* ====== Tour Links Section ====== \*\/\}/;
// And the tour block
const tourRegex = /\{\/\* ====== Tour Links Section ====== \*\/\}[\s\S]*?\{\/\* ====== FAQs Section ====== \*\/\}/;
...
code = code.replace(marketingRegex, '');
code = code.replace(tourRegex, combinedBlock);

fs.writeFileSync('client/src/pages/BhutanConsultingPage.jsx', code);
console.log('Bhutan layout combined and image preview added!');
