...
// 1. Fix Imports
code = code.replace(
    /import { Briefcase, HeartPulse, CloudSun, BookKey as Passport, Search, Copy, CheckCircle2, ChevronDown, Hash, PhoneCall, Sparkles, MessageSquare, Info, Zap, ChevronRight } from 'lucide-react';/,
...
// 2. Insert referenceLinks before groupedFaqs
...
`;
code = code.replace('const groupedFaqs = [', fullArray + 'const groupedFaqs = [');


// 3. Insert copiedLink state and handleCopyLink
...
`;
code = code.replace('    const normalizedSearch = searchTerm.toLowerCase();', filterLogic);
...
code = code.replace('            case "Passport": return <Passport size={20} />;', '            case "Passport": return <Passport size={20} />;' + getIconAdd);


// 6. Insert original UI blocks
...
console.log("Restored perfectly from clean state.");
