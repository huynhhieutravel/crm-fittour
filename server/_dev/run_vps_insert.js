...
// Check VPS path first, then fallback to local paths
...
<p>Đề nghị tất cả các BU và các bộ phận liên quan nghiêm túc quán triệt và triển khai thực hiện ngay kể từ ngày ban hành thông báo này.</p>`;
        ...
        // Also seed other demo announcements if none
        await client.query(`
            INSERT INTO official_announcements (
                code, title, category, issue_date, effective_date, signer_name, signer_position,
                recipient_scope, summary, content_html, is_pinned, status
            ) VALUES (
                'QĐ-2026/08/01-FIT',
                'Ban hành Quy chế Đánh giá Hiệu suất & Cơ chế Thưởng Nóng Khối Kinh Doanh (BU)',
                'Quyết định',
                '2026-08-15',
                '2026-08-15',
                'Nguyễn Nhất Vũ',
                'Tổng Giám Đốc',
                'Khối Kinh Doanh (BU1, BU2, BU3, BU4, BU5)',
                'Quyết định áp dụng cơ chế thưởng nóng doanh số vượt chỉ tiêu dành cho Sales và Trưởng nhóm kinh doanh trong quý 3/2026.',
                '<p>Ban Giám Đốc quyết định ban hành cơ chế thưởng nóng...</p>',
                false,
                'published'
            ) ON CONFLICT (code) DO NOTHING;
        `);

        console.log('🎉 Done inserting on DB!');
    } catch (err) {
