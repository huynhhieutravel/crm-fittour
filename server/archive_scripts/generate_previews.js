...
async function generate(type, month, year, week) {
  ...
  const aggResult = await pool.query(aggQuery, queryParams);
  ...
  const buBreakdownHtml = activeBUs.map(bu => {
    ...
    // For weekly, the target leads and budget should be proportional if we want to show it,
    // but the original code just uses the monthly targets in the breakdown. Wait, no.
    // The original code actually does:
    totalBudget += (type === 'weekly' ? budget / 4.33 : budget); 
    // Wait, let's just use the logic from the route.
    totalBudget += budget;
    ...
    // For weekly, we divide budget and target leads by 4.33 approx for display?
    const displayBudget = type === 'weekly' ? budget / 4.33 : budget;
    const displayTargetLeads = type === 'weekly' ? targetLeads / 4.33 : targetLeads;
    ...
    return `
      <tr>
        ...
        <td style="padding: 10px; border-bottom: 1px solid #e2e8f0;">${actual.spend.toLocaleString('vi-VN')} / ${Math.round(displayBudget).toLocaleString('vi-VN')}</td>
        <td style="padding: 10px; border-bottom: 1px solid #e2e8f0;">${actual.leads} / ${Math.round(displayTargetLeads)}</td>
  ...
  const displayTotalBudget = type === 'weekly' ? totalBudget / 4.33 : totalBudget;
  const displayTotalTargetLeads = type === 'weekly' ? totalTargetLeads / 4.33 : totalTargetLeads;
  const budgetUsagePercent = displayTotalBudget > 0 ? ((totalSpend / displayTotalBudget) * 100).toFixed(1) : 0;
    ...
    <div style="font-family: sans-serif; max-width: 800px; margin: 0 auto; background: #fff; border: 1px solid #e2e8f0; border-radius: 8px; padding: 20px;">
      ...
      <div style="display: flex; justify-content: space-between; margin-bottom: 20px; gap: 15px; flex-wrap: wrap;">
         ...
         <div style="flex: 1; min-width: 150px; background: #f8fafc; padding: 15px; border-radius: 8px; border: 1px solid #e2e8f0;">
            ...
            <div style="font-size: 20px; color: #f59e0b; font-weight: bold; margin-top: 5px;">${totalActualLeads} / ${Math.round(displayTotalTargetLeads)}</div>
         </div>
         <div style="flex: 1; min-width: 150px; background: #f8fafc; padding: 15px; border-radius: 8px; border: 1px solid #e2e8f0;">
...
async function run() {
  const mHtml = await generate('monthly', 7, 2026);
  const wHtml = await generate('weekly', 7, 2026, 1);
  
  fs.writeFileSync('/Users/huynhtronghieu/.gemini/antigravity-ide/brain/2d4ee57c-e269-44ac-a7c5-8cd8e9959e0b/monthly_preview.html', mHtml);
  fs.writeFileSync('/Users/huynhtronghieu/.gemini/antigravity-ide/brain/2d4ee57c-e269-44ac-a7c5-8cd8e9959e0b/weekly_preview.html', wHtml);
