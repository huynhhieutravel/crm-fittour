    ...
    try {
        ...
        const sql = `
            SELECT t.*, 
                   ...
                   (SELECT COUNT(*)::int FROM bookings WHERE customer_id = t.customer_id AND booking_status NOT IN ('Huỷ', 'Hủy', 'Mới', 'CANCELLED', 'EXPIRED')) as total_bookings
            FROM cskh_tasks t 
            JOIN customers c ON t.customer_id = c.id
    ...
    try {
        ...
        if (min_trips) {
            conditions.push(`(
                ...
                COALESCE((SELECT COUNT(*)::int FROM bookings WHERE customer_id = c.id AND booking_status NOT IN ('Huỷ', 'Hủy', 'Mới', 'CANCELLED', 'EXPIRED')), 0)
            ) >= $${idx++}`);
