...
const files = [
  { week: 5, name: 'tuan5-thang4-nam2026.xlsx' }
];
...
const processFiles = async () => {
  for (let fileObj of files) {
    ...
    await pool.query('DELETE FROM marketing_ads_reports WHERE year = 2026 AND month = 4 AND week_number = $1', [fileObj.week]);
    ...
    for (const row of data) {
      ...
      if (bu !== 'UNKNOWN' && (spend > 0 || campaign)) {
        ...
        await pool.query(
          ...
          [bu, 2026, 4, fileObj.week, row['Tên chiến dịch'] || '', row['Tên nhóm quảng cáo'] || '', row['Tên quảng cáo'] || '', spend, msgs, leads, cplMsg, cplLead]
        );
        count++;
      }
    }
    console.log(`Tuần ${fileObj.week} tháng 4: Đã thêm ${count} dòng quảng cáo hợp lệ.`);
  }
