...
const processFiles = async () => {
  for (let fileObj of files) {
    ...
    await pool.query('DELETE FROM marketing_ads_reports WHERE year = 2026 AND month = 2 AND week_number = $1', [fileObj.week]);
    ...
    for (const row of data) {
      ...
      if (!bu) {
        ...
        else if (allText.includes('ALASKA') || allText.includes('NAM MỸ') || allText.includes('CHÂU ÂU') || allText.includes('SILKROAD')) bu = 'BU2';
      ...
      if (bu !== 'UNKNOWN' && (spend > 0 || campaign)) {
        ...
        await pool.query(
          ...
          [bu, 2026, 2, fileObj.week, row['Tên chiến dịch'] || '', row['Tên nhóm quảng cáo'] || '', row['Tên quảng cáo'] || '', spend, msgs, leads, cplMsg, cplLead]
        );
        count++;
      }
    }
    console.log(`Tháng 2 - Tuần ${fileObj.week}: Đã thêm ${count} dòng quảng cáo hợp lệ.`);
  }
