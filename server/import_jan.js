...
const files = [
  { week: 1, name: 'tuan1-thang1-nam2026.xlsx' },
  { week: 2, name: 'tuan2-thang1-nam2026.xlsx' },
  { week: 3, name: 'tuan3-thang1-nam2026.xlsx' },
  { week: 4, name: 'tuan4-thang1-nam2026.xlsx' },
  { week: 5, name: 'tuan5-thang1-nam2026.xlsx' }
];
...
const processFiles = async () => {
  for (let fileObj of files) {
    ...
    await pool.query('DELETE FROM marketing_ads_reports WHERE year = 2026 AND month = 1 AND week_number = $1', [fileObj.week]);
    ...
    for (const row of data) {
      ...
      if (!bu) {
        ...
        if (allText.includes('TRUNG QUỐC') || allText.includes('BẮC KINH') || allText.includes('THƯỢNG HẢI') || allText.includes('Á ĐINH') || allText.includes('GIANG NAM') || allText.includes('LỆ GIANG') || allText.includes('GIANG TÂY')) bu = 'BU1';
        else if (allText.includes('ALASKA') || allText.includes('NAM MỸ') || allText.includes('CHÂU ÂU') || allText.includes('SILKROAD') || allText.includes('AI CẬP')) bu = 'BU2';
      ...
      if (bu !== 'UNKNOWN' && (spend > 0 || campaign)) {
        ...
        await pool.query(
          ...
          [bu, 2026, 1, fileObj.week, row['Tên chiến dịch'] || '', row['Tên nhóm quảng cáo'] || '', row['Tên quảng cáo'] || '', spend, msgs, leads, cplMsg, cplLead]
        );
        count++;
      }
    }
    console.log(`Tháng 1 - Tuần ${fileObj.week}: Đã thêm ${count} dòng quảng cáo hợp lệ.`);
  }
