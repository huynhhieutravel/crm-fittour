...
const files = [
  { week: 3, name: 'tuan3-thang5-nam2026.xlsx' },
  { week: 4, name: 'tuan4-thang5-nam2026.xlsx' }
];
...
const processFiles = async () => {
  for (let fileObj of files) {
    ...
    await pool.query('DELETE FROM marketing_ads_reports WHERE year = 2026 AND month = 5 AND week_number = $1', [fileObj.week]);
    ...
    for (const row of data) {
      ...
      if (!bu) {
        ...
        if (allText.includes('TRUNG QUỐC') || allText.includes('BẮC KINH') || allText.includes('THƯỢNG HẢI') || allText.includes('Á ĐINH') || allText.includes('GIANG NAM') || allText.includes('TÂN CƯƠNG') || allText.includes('TÂY TẠNG') || allText.includes('LỆ GIANG')) bu = 'BU1';
        else if (allText.includes('ALASKA') || allText.includes('NAM MỸ') || allText.includes('CHÂU ÂU') || allText.includes('ÚC') || allText.includes('MỸ') || allText.includes('CANADA')) bu = 'BU2';
        else if (allText.includes('HÀN QUỐC') || allText.includes('NHẬT BẢN') || allText.includes('ĐÀI LOAN')) bu = 'BU3';
        else if (allText.includes('BALI') || allText.includes('BHUTAN') || allText.includes('LADAKH') || allText.includes('BROMO') || allText.includes('KASHMIR') || allText.includes('MÔNG CỔ') || allText.includes('ẤN ĐỘ')) bu = 'BU4';
      }
      ...
      if (bu !== 'UNKNOWN' && (spend > 0 || campaign)) {
        ...
        await pool.query(
          ...
          [bu, 2026, 5, fileObj.week, row['Tên chiến dịch'] || '', row['Tên nhóm quảng cáo'] || '', row['Tên quảng cáo'] || '', spend, msgs, leads, cplMsg, cplLead]
        );
        count++;
      }
    }
    console.log(`Tuần ${fileObj.week} tháng 5: Đã thêm ${count} dòng quảng cáo hợp lệ.`);
  }
