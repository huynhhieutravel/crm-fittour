...
router.post('/send-email', async (req, res) => {
  try {
    ...
    // Get email rule
    const eventCode = type === 'weekly' ? 'WEEKLY_MARKETING_ADS_STATS' : 'MONTHLY_MARKETING_ADS_STATS';
    const ruleRes = await db.query('SELECT * FROM email_rules WHERE event_code = $1 AND is_active = true', [eventCode]);
    
    if (ruleRes.rows.length === 0) {
      return res.status(400).json({ error: 'Chưa cấu hình rule gửi email cho sự kiện này hoặc đã bị tắt.' });
    }
    const rule = ruleRes.rows[0];

    // Helper for week ranges
    ...
    let dateString = `Tháng ${targetMonth}/${targetYear}`;
    if (type === 'weekly') {
      const ranges = getWeekRanges(targetYear, targetMonth);
      const rangeStr = ranges[week] ? ` (${ranges[week]}) ` : ' ';
      dateString = `Tuần ${week}${rangeStr}Tháng ${targetMonth}/${targetYear}`;
    }
    ...
    const buBreakdownHtml = activeBUs.map(bu => {
      ...
      return `
        <tr>
          ...
          <td style="padding: 10px; border-bottom: 1px solid #e2e8f0;">${spendHtml}</td>
          <td style="padding: 10px; border-bottom: 1px solid #e2e8f0;">${actual.messages.toLocaleString('vi-VN')} / ${formatK(cpm)}</td>
          <td style="padding: 10px; border-bottom: 1px solid #e2e8f0;">${actual.leads} / ${targetLeads} <span style="color: #64748b;">(${targetLeads > 0 ? Math.round((actual.leads / targetLeads) * 100) : 0}%)</span></td>
    ...
    const html = `
        ...
        <div style="margin-top: 40px; font-size: 10px; color: #cbd5e1; text-align: center;">
          Báo cáo tạo lúc: ${new Date().toLocaleString('vi-VN', { timeZone: 'Asia/Ho_Chi_Minh' })} (Tránh Gmail nhóm email)
        </div>
    `;
    ...
    console.error('Lỗi khi gửi báo cáo ads:', error);
