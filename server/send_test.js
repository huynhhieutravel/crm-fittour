...
async function run() {
  ...
  `;
  const aggResult = await pool.query(aggQuery, [targetYear, targetMonth, week]);
  ...
  const buBreakdownHtml = activeBUs.map(bu => {
    ...
    let budget = parseFloat(kpi.budget || 0);
    ...
    if (isThieuLead && isCpaVuot) {
      ...
      statusColor = '#f59e0b';
      alerts.push(`⚠️ <b>${bu.id}:</b> Thiếu ${shortfallLeads} Lead so với Target tuần.`);
    } else if (isCpaVuot) {
    ...
    if (budget > 0) {
      ...
      if (budgetUsage > 0.8 && leadProgress < 0.6) {
         alerts.push(`💸 <b>${bu.id}:</b> Đã tiêu >80% Ngân sách tuần nhưng lượng Lead mới đạt ${(leadProgress*100).toFixed(0)}%.`);
      }
    }
  ...
  const alertsHtml = alerts.length > 0 
    ...
    : `<div style="background: #ecfdf5; border-left: 4px solid #10b981; padding: 15px; margin-bottom: 20px; border-radius: 4px; color: #065f46; font-weight: 500;">
         🎉 Tuyệt vời! Các BU đang hoạt động ổn định và giữ vững KPI tuần.
       </div>`;
  ...
  const html = `
    <div style="font-family: sans-serif; max-width: 800px; margin: 0 auto; background: #fff; border: 1px solid #e2e8f0; border-radius: 8px; padding: 20px;">
      <h2 style="color: #1e293b; margin-top: 0; border-bottom: 2px solid #3b82f6; padding-bottom: 10px;">Báo Cáo Marketing Ads - Tuần 4 Tháng ${targetMonth}/${targetYear} (BẢN CUỐI)</h2>
  ...
  await queueService.enqueue('send-email', {
    recipient: 'huynhhieutravel@gmail.com',
    subject: `[Bản hoàn thiện cuối cùng] Báo Cáo Marketing Ads - Tuần 4 Tháng 7/2026`,
    bodyHtml: html,
    from: '"[ERP FIT Tour]" <loki@fittour.vn>'
  }, { retryLimit: 1 });
  
  console.log('Test email enqueued successfully. Waiting 5s for worker to process...');
  setTimeout(() => {
    console.log('Exiting send_test.js');
    process.exit(0);
  }, 5000);
}
